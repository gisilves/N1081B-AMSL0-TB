...
def reset_scalers():
    ...
    now = datetime.now()
    print_and_log(now.strftime("%d/%m/%Y %H:%M:%S") + "\tScalers reset before calibration trigger")
...
def enable_calibration_trigger():
    ...
    while goodconnection1==False or goodconnection2==False: 
        ...
        if (authorized1 and authorized2):
            ...
            now = datetime.now()
            print_and_log(now.strftime("%d/%m/%Y %H:%M:%S") + "\tStarting CAL trigger ...")
            ...
            CT_send_run_cmd("START", 0, "/Data/BLOCKS/USBLF_PCGSC03/", logfile_path)
            
            enable_calibration()
            
            now = datetime.now()
            print_and_log(now.strftime("%d/%m/%Y %H:%M:%S") + "\tCAL triggers enabled")
